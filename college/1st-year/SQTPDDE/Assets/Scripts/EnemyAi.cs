...
public class EnemyAi : MonoBehaviour
{
    ...
    private void Update()
    {
        if (health <= 0)
        {
            GameData.SetHasBeatenBoss(bossKey, true);
    ...
    private void AttackPlayer()
    {
        ...
        if (!alreadyAttacked)
        {   
            rigidBody.AddForce(transform.forward * 6f, ForceMode.Impulse);
            ///Attack code here
            Rigidbody rb = Instantiate(projectile, spawnerProjectile.transform.position, Quaternion.identity).GetComponent<Rigidbody>();
