...
hyperfine_benchmark() {
    ...
    for input in ../data/*.ppm; do
        for thread in 1 2 4 8 16 32; do
            echo "[*] Benchmarking blur ($input) Threads: $thread"
            hyperfine --warmup 10 --output=pipe "./$blur_bin 15 $input $blur_tmp $thread" >> $output_blur_stats
        done
    done
    # fi

    for input in ../data/*.ppm; do
       for thread in 1 2 4 8 16 32; do # 2 4 8 16 32
            echo "[*] Benchmarking threshold ($input) Threads: $thread"
            hyperfine --warmup 40 --output=pipe "./$threshold_bin $input $threshold_tmp $thread" >> $output_threshold_stats
          done
    done
